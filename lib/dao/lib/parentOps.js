...
const internals = {
    personParentQuery: "start p=node({id}) match (p)-[r]->(n) where type(r) IN ['child_of'] return n"
};
...
exports.find = (conn, personId) => {
    return Q.nfcall(conn.query, internals.personParentQuery, {id: personId});
};
 ...
 *                               -13 : forbeen parents relation not respected
 */
internals.canBeParent = (conn, person, parent) => {
    ...
    exports.find(conn, person.id).then(
        ...
        (parents) => {
            ...
            //check forbedden parents rules
            return internals.checkForbeddenParentRules(conn,person,parent);
        },
        () => {
            deferred.reject(new Error(-3));
        }
...
/**
 * Checks if the given person and parents respect the rule of the forbidden parent or not.
 * @param conn Connection to the Neo4j database
 * @param person The person
 * @param parent The parent of the person
 * @returns {*|promise}
 *          promise resolved --> the rule is respected
 *          promise rejected --> the rule is not respected
 */
internals.checkForbeddenParentRules = (conn, person, parent) =>  {

    const deferred = Q.defer();
    const forbeddenParents = internals.config.get('forbeddenParent', 'rules');
    const promises = [];

    forbeddenParents.map(rel => {
        promises.push(internals.checksPersonsRelation(conn, person.id, rel, parent.id));
    });
 ...
 *          --
 */
internals.checksPersonsRelation = (conn, personOneId, relation, personTwoId) => {
    const deferred = Q.defer();
