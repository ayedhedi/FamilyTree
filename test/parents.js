...
lab.experiment('parents', () => {
    ...
    const relations = ['grand parent','grand childrens','spouses','siblings','uncles',
        'aunts','cousins','nephews','nieces','siblingsInLaw','daughtersInLaw','sonsInLaw','fathersInLaw','mothersInLaw'];
    const persons = ['F','A','B','H','G','F','F','C','C','C','A','A','D','C'];
    const parents = ['A','F','D','F','B','C','G','M','F','D','D','E','A','K'];
    ...
    for(var i=0;i<relations.length;i++){
        ...
        lab.test(relation, () => {
            return internals.dao.createParent(internals.persons[person].id, internals.persons[parent].id).then(
                () => {
                    Code.fail(person+" cannot be the parent of "+parent);
                },
                (err) => {
